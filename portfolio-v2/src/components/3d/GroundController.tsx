...
export function GroundController() {
  ...
  useFrame((state, delta) => {
    ...
    const steeringEffectiveness =
      1 - (groundSpeedRef.current / GROUND_PHYSICS.maxGroundSpeed) * 0.7;
    const targetSteering =
      input.yaw * GROUND_PHYSICS.maxSteeringAngle * steeringEffectiveness;

    // Smooth steering
    steeringRef.current +=
      (targetSteering - steeringRef.current) * GROUND_PHYSICS.steeringRate * dt;

    // Apply steering to heading
    if (groundSpeedRef.current > 1) {
      group.rotation.y +=
        steeringRef.current * dt * (groundSpeedRef.current / 30);
    }
    ...
    const groundHeight =
      ...
      Math.sin(state.clock.elapsedTime * 20) *
        (groundSpeedRef.current / 200) *
        0.1;
    group.position.y = groundHeight;

    // === LIFTOFF CHECK ===
    if (
      groundSpeedRef.current >= GROUND_PHYSICS.liftoffSpeed &&
    ...
    const idealCamPos = group.position
      .clone()
      .add(
        new THREE.Vector3(
          0,
          10 + groundSpeedRef.current * 0.05,
          35 + groundSpeedRef.current * 0.2
        )
      );
