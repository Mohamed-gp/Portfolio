  ...
  return (
    <div className="relative w-28 h-28 rounded-full overflow-hidden border-2 border-emerald-500/50 shadow-lg shadow-emerald-500/20">
      ...
      <motion.div
        ...
        style={{
          background: 'linear-gradient(to bottom, #0c4a6e 0%, #0c4a6e 50%, #78350f 50%, #78350f 100%)',
          rotate: roll,
          y: pitch * 1.5,
        }}
      />
      ...
      >
        ...
        {[-30, -20, -10, 10, 20, 30].map(deg => (
          ...
          >
            <div className={`flex items-center gap-1 ${deg > 0 ? 'text-cyan-400' : 'text-amber-600'}`}>
              <div className={`w-3 h-0.5 ${deg > 0 ? 'bg-cyan-400' : 'bg-amber-600'}`} />
              <span className="text-[8px] font-mono opacity-80">{Math.abs(deg)}</span>
              <div className={`w-3 h-0.5 ${deg > 0 ? 'bg-cyan-400' : 'bg-amber-600'}`} />
            </div>
          </motion.div>
        ))}
        ...
        {/* Main horizon line */}
        <div className="absolute top-1/2 left-1/4 right-1/4 h-0.5 bg-white -translate-y-1/2" style={{ transform: `translateY(${pitch * 1.5}px)` }} />
      </motion.div>
      
      {/* Aircraft reference symbol (fixed) */}
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <svg width="60" height="30" viewBox="0 0 60 30" className="text-amber-400">
          {/* Wings */}
          <line x1="0" y1="15" x2="22" y2="15" stroke="currentColor" strokeWidth="3" />
          <line x1="38" y1="15" x2="60" y2="15" stroke="currentColor" strokeWidth="3" />
          {/* Center */}
          <circle cx="30" cy="15" r="4" fill="currentColor" />
          {/* Tail */}
          <line x1="30" y1="19" x2="30" y2="26" stroke="currentColor" strokeWidth="2" />
        </svg>
      </div>
      ...
      <div className="absolute bottom-1 left-1 bg-black/60 rounded px-1">
        <span className="text-[8px] font-mono text-emerald-400">{Math.round(pitch)}°</span>
      </div>
      <div className="absolute bottom-1 right-1 bg-black/60 rounded px-1">
        <span className="text-[8px] font-mono text-cyan-400">{Math.round(roll)}°</span>
      </div>
    </div>
  );
}
    ...
    <div className="relative w-16 h-16 rounded-full border border-emerald-500/40 bg-slate-900/80">
      ...
      <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 bg-black/60 rounded px-1">
        <span className={`text-[9px] font-mono ${
          vs > 500 ? 'text-cyan-400' : vs < -500 ? 'text-amber-400' : 'text-emerald-400'
        }`}>
          {vs > 0 ? '+' : ''}{Math.round(vs)}
        </span>
      </div>
...
function GForceMeter({ g }: { g: number }) {
  ...
  return (
    <div className="bg-slate-900/80 border border-emerald-500/30 rounded-lg p-2 w-20">
      <div className="text-slate-500 font-mono text-[8px] text-center">G-LOAD</div>
      <div className={`font-mono text-2xl font-bold text-center ${colors.text}`}>
        {g.toFixed(1)}
      </div>
      {/* G meter bar */}
...
function MachIndicator({ mach }: { mach: number }) {
  ...
  return (
    <div className="bg-slate-900/80 border border-emerald-500/30 rounded-lg p-2 w-20">
      <div className="text-slate-500 font-mono text-[8px] text-center">MACH</div>
      <div className={`font-mono text-2xl font-bold text-center ${
        isSupersonic ? 'text-cyan-400' : isTransonic ? 'text-amber-400' : 'text-emerald-400'
      }`}>
        {mach.toFixed(2)}
      </div>
      {isSupersonic && (
        <div className="text-[8px] text-cyan-400 text-center animate-pulse">SUPERSONIC</div>
      )}
    </div>
  );
}
...
function EngineStatus({ temp, oil }: { temp: number; oil: number }) {
  return (
    <div className="bg-slate-900/80 border border-emerald-500/30 rounded-lg p-2">
      <div className="text-slate-500 font-mono text-[8px] text-center mb-1">ENGINE</div>
      <div className="flex gap-3">
        <div className="flex flex-col items-center">
          <span className="text-[7px] text-slate-500">EGT</span>
          <span className={`text-xs font-mono ${temp > 850 ? 'text-red-500' : temp > 700 ? 'text-amber-400' : 'text-emerald-400'}`}>
            {Math.round(temp)}°
          </span>
        </div>
        <div className="flex flex-col items-center">
          <span className="text-[7px] text-slate-500">OIL</span>
          <span className={`text-xs font-mono ${oil < 30 ? 'text-red-500' : oil < 50 ? 'text-amber-400' : 'text-emerald-400'}`}>
            {Math.round(oil)}
          </span>
        </div>
      </div>
    ...
    <div className="bg-slate-900/80 border border-emerald-500/30 rounded-lg p-2 w-24">
      ...
      <div className="relative h-48 overflow-hidden mt-1">
          ...
          {tapeMarks.map(({ speed: markSpeed, offset }) => (
            ...
            >
              <span className={`text-[10px] font-mono mr-1 ${
                markSpeed === Math.round(displaySpeed / 10) * 10 ? 'text-white' : 'text-slate-500'
              }`}>
                {markSpeed}
              </span>
              <div className={`w-3 h-px ${markSpeed % 50 === 0 ? 'bg-emerald-500' : 'bg-slate-600'}`} />
            </motion.div>
          ))}
        </div>
        ...
        <div className={`absolute left-0 right-0 top-1/2 -translate-y-1/2 flex items-center justify-center ${
          isLowSpeed ? 'bg-red-500/30' : isHighSpeed ? 'bg-amber-500/30' : 'bg-emerald-500/30'
        } rounded py-1`}>
          <span className={`font-mono text-2xl font-bold ${
            isLowSpeed ? 'text-red-400' : isHighSpeed ? 'text-amber-400' : 'text-white'
          }`}>
            {displaySpeed}
          </span>
        </div>
      ...
      <div className="text-emerald-400 font-mono text-[8px] text-center mt-1">KNOTS</div>
    </div>
  );
}

// Altitude Tape (vertical)
function AltitudeTape({ altitude, maxAlt }: { altitude: number; maxAlt: number }) {
  ...
  return (
    <div className="bg-slate-900/80 border border-cyan-500/30 rounded-lg p-2 w-24">
        ...
        <div className="absolute inset-0 flex flex-col items-start justify-center">
          {tapeMarks.map(({ alt: markAlt, offset }) => (
            <motion.div
              ...
              style={{ top: `calc(50% - ${offset}px)` }}
            >
              <div className={`w-3 h-px ${markAlt % 1000 === 0 ? 'bg-cyan-500' : 'bg-slate-600'}`} />
              <span className={`text-[10px] font-mono ml-1 ${
                markAlt === Math.round(displayAlt / 100) * 100 ? 'text-white' : 'text-slate-500'
              }`}>
      ...
      <div className="text-cyan-400 font-mono text-[8px] text-center mt-1">FEET MSL</div>
    </div>
  );
}
          ...
          {Array.from({ length: 73 }).map((_, i) => {
            ...
            return (
              <div key={i} className="flex flex-col items-center w-10">
                <div className={`h-${isCardinal ? '4' : isMajor ? '3' : '2'} w-px ${
                  isCardinal ? 'bg-amber-400' : isMajor ? 'bg-emerald-500' : 'bg-slate-600'
                }`} style={{ height: isCardinal ? '16px' : isMajor ? '12px' : '8px' }} />
                {(isCardinal || isMajor) && (
                  <span className={`text-[10px] font-mono ${isCardinal ? 'text-amber-400 font-bold' : 'text-emerald-500'}`}>
                    {label}
                  </span>
                )}
              </div>
...
export function HUD() {
  const { speed, altitude, heading, currentIsland, currentAircraft, showHUD, isLanded } = useGameStore();
  ...
  useEffect(() => {
    ...
    setTelemetry(prev => ({
      verticalSpeed,
      gForce: THREE.MathUtils.lerp(prev.gForce, Math.max(-3, Math.min(9, gForce)), 0.3),
      mach,
      aoa,
    ...
    <div className="pointer-events-none fixed inset-0 z-30 font-mono">
        ...
        {isLowSpeed && !isLanded && (
          ...
          >
            <div className="flex items-center gap-3 rounded-lg border-2 border-red-500 bg-red-500/20 px-6 py-3 animate-pulse">
              ...
              <div>
                <div className="text-red-500 font-bold text-xl">STALL WARNING</div>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <EngineStatus temp={telemetry.engineTemp} oil={telemetry.oilPressure} />
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {currentIsland && (
            <div className="rounded bg-slate-800/80 border border-cyan-500/30 px-3 py-1.5">
              ...
              <span className="text-cyan-400 font-bold">{ISLANDS[currentIsland].name}</span>
            </div>
          )}
        </div>
      </motion.div>
          ...
          >
            <div className="rounded-lg border-2 border-emerald-500 bg-slate-900/90 px-6 py-3 text-center">
              ...
              <div className="flex items-center justify-center gap-4 text-sm mb-2">
                <span className={speed < 0.1 ? 'text-emerald-400' : 'text-amber-400'}>
                  SPD: {speed < 0.1 ? '✓' : '↓'} {Math.round(speed * 340)}
                </span>
                <span className="text-slate-500">|</span>
                <span className={altitude < 80 ? 'text-emerald-400' : 'text-amber-400'}>
              ...
              {speed < 0.1 ? (
                ...
                <div className="text-amber-400 text-sm">Reduce speed below 35 KTS</div>
              )}
            </div>
          </motion.div>
        )}
      ...
      >
        <div className="flex gap-4 rounded-lg bg-slate-900/80 border border-slate-700 px-4 py-2 text-xs text-slate-400">
          <span><kbd className="rounded bg-slate-700 px-1.5 py-0.5 text-emerald-400 mr-1">W</kbd><kbd className="rounded bg-slate-700 px-1.5 py-0.5 text-emerald-400">S</kbd> Pitch</span>
          <span><kbd className="rounded bg-slate-700 px-1.5 py-0.5 text-emerald-400 mr-1">A</kbd><kbd className="rounded bg-slate-700 px-1.5 py-0.5 text-emerald-400">D</kbd> Roll</span>
          <span><kbd className="rounded bg-slate-700 px-1.5 py-0.5 text-emerald-400 mr-1">Q</kbd><kbd className="rounded bg-slate-700 px-1.5 py-0.5 text-emerald-400">E</kbd> Yaw</span>
          <span><kbd className="rounded bg-slate-700 px-1.5 py-0.5 text-cyan-400">SHIFT</kbd> Afterburner</span>
          <span><kbd className="rounded bg-slate-700 px-1.5 py-0.5 text-amber-400">SPACE</kbd> Land</span>
        </div>
      </motion.div>
    </div>
  );
