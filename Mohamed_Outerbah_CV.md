...
- **Traction:** Reached 1,100+ active stores, 2,500+ products, 650+ orders, and 5.5M+ DZD in merchant sales (GMV) in the 3 months since the May 2026 launch, on 47,000+ visitors and 166,000+ pageviews, still adding ~160 new stores and 200+ orders a week with new stores up 88% and orders up 270% month over month, 100% organic with zero ad spend, and **25+ merchants upgraded to paid PRO plans, acquired through SEO alone**.
- **Quality signals:** 4.7/5 average across 73 in-app merchant survey responses; 99.95% uptime over the last 90 days with zero recorded incidents (public status page: [status.dzstore.org](https://status.dzstore.org)).
...
- **Revenue protection:** Noticed free merchants with heavy order volume were not upgrading, audited the free-plan order cap end to end, and found customer contact details bypassing it through 8 server-side channels (order detail API, CSV and carrier exports, email, WhatsApp and push alerts, Google Sheets webhook, customers list); closed every leak behind a single order-privacy module, pinned by regression tests that scan whole payloads for raw customer data.
