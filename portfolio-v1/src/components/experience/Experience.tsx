...
export default function Experience() {
  ...
  const workExperience = [
    ...
    },
    {
      ...
      description:
        "Co-founded a Shopify-equivalent multi-tenant e-commerce SaaS and lead nearly all engineering across the stack. Grown in its first 3 months live to 1,000+ active stores and 20+ merchants upgraded to paid PRO plans through SEO alone, 100% organic with zero ad spend, plus 5.5M+ DZD in merchant sales (GMV) and 99.95% uptime over the last 90 days. Multi-tenant subdomain-per-store routing with Caddy on-demand TLS, a 44-model Prisma/PostgreSQL schema, online card payments (Chargily Pay) with HMAC-verified webhooks, a carrier layer covering 70 Algerian delivery companies behind 5 API adapters, and technical SEO with 15,000+ pages indexed, a Google Merchant Center feed (2,600+ product listings), and Meta Pixel integration pulling 3,000+ Google Search clicks a month; plus a React Native (Expo) merchant app with real-time order tracking, push notifications, and stock management; self-hosted on Hetzner via Docker with zero-downtime deploys. Cut the database bill by 50%+ by migrating the production Postgres from Neon to Supabase with pg_dump at zero downtime, and retired ~17 GB/month of CDN egress by persisting the image-optimizer cache across deploys.",
      skills: [
        "Next.js",
        "React Native (Expo)",
        "TypeScript",
